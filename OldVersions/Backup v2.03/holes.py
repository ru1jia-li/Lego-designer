...
class HoleManagerDialog(QDialog):
    ...
    def auto_detect_holes(self):
        ...
        # Auto-detect is an edit → snapshot
        self._push_snapshot()
    ...
    def eventFilter(self, source, event):
        ...
        if event.type() == QEvent.Type.MouseButtonRelease and self._move_undo_armed:
            # move finished; nothing more to do (snapshot already taken)
            self._move_undo_armed = False
    ...
    def save_holes(self):
        ...
        if len(holes) == 0:
            #print("[SAVE HOLES] No holes found → not overwriting JSON")
            return
    ...
    def undo(self):
        if self.undo_index <= self.baseline_index:
            return
        self.undo_index -= 1
    ...
    def paste(self):
        ...
        for px, py, r in self.copied_holes:
            new_center = QPointF(px + 10, py + 10)
            dot = self._create_hole_item(new_center, r)
            new_items.append(dot)
